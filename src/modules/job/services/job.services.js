...
export const addJob = asyncHandller(async (req, res, next) => {
    ...
    if (!(company.HRs?.some(hrId => hrId.toString() === req.user._id.toString()) || company.createdBy.toString() === req.user._id.toString())) {
        return next(new Error("you are not the owner or hr in this company so you can't add job!!", { cause: 403 }))
    }
...
export const deleteJob = asyncHandller(async (req, res, next) => {
    ...
    if (!company.HRs?.some(hrId => hrId.toString() === req.user._id.toString())) {
        return next(new Error("you are not hr in this company so you can't delete job!!", { cause: 403 }))
    }
    const job = await DbServices.findOneAndUpdate({
        model: jobModel,
        filter: {
...
export const getAllJobs = asyncHandller(async (req, res, next) => {
    ...
    const companyNameExist = await DbServices.findOne({
        model: companyModel,
        filter: {
...
export const applyToJob = asyncHandller(async (req, res, next) => {
    ...
    if ((company.HRs?.some(hrId => hrId.toString() === req.user._id.toString()) || company.createdBy.toString() === req.user._id.toString())) {
        return next(new Error("owner and Hrs can't apply to jobs!!", { cause: 403 }))
    }
    const job = await DbServices.findOne({
        model: jobModel,
        filter: {
...
export const getAllApplications = asyncHandller(async (req, res, next) => {
    ...
    if (!(company.HRs?.some(hrId => hrId.toString() === req.user._id.toString()) || company.createdBy.toString() === req.user._id.toString())) {
        return next(new Error("you are not the owner or hr in this company so you can't check the applications!!", { cause: 403 }))
    }
    const job = await DbServices.findOne({
        model: jobModel,
        filter: {
...
export const acceptOrRejectApplicant = asyncHandller(async (req, res, next) => {
    ...
    const application = await DbServices.findOne({
        ...
        populate: [
            ...
            },
            {
                ...
                populate: [{
                    path: "companyId",
                    select: "companyName HRs createdBy"
                }]
    ...
    if (!(application.jobId?.companyId?.HRs?.some(hrId => hrId.toString() === req.user._id.toString()))) {
        return next(new Error("you are not hr so you can't accept or reject the applications!!", { cause: 403 }))
    }
