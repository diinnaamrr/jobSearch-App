...
const isCompanyName=await DbServices.findOne({model:companyModel,
    filter:{companyName}
})
if(isCompanyName){
    return next(new Error("this name is already exist",{cause:409}))
}
const isCompanyEmail=await DbServices.findOne({model:companyModel,
    filter:{companyEmail}
})
if(isCompanyEmail){
...
const company = await DbServices.create({model:companyModel,
    data:{
        companyEmail,
        companyName,
        address,
...
const companyExist=await DbServices.findOne({model:companyModel,
    filter:{
        _id:companyId,
        deletedAt:{$exists:false},
...
const company=await DbServices.findOneAndUpdate({model:companyModel,
    filter:{
        _id:companyId,
        deletedAt:{$exists:false},
        approvedByAdmin:true,
...
await DbServices.updateOne({model:companyModel,
    filter:{_id:companyId,
    daletedAt:{$exists:false},
    bannedAt:{$exists:false},
    approvedByAdmin:true,
